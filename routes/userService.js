...
export const getUser = async (id, options = {}) => {
  ...
  // agar select diya gaya hai to usko use karo warna default fields + profilePicture
  if (select) {
    query = query.select(select);
  } else {
    query = query.select("name email profilePicture");
  }
  ...
  // local system me stored profile picture ka full URL generate karo
  if (user.profilePicture) {
    user.profilePictureUrl = `/uploads/${user.profilePicture}`;
  }
