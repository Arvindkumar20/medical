...
const doctorProfileSchema = new mongoose.Schema(
  {
    ...
    consultationTypes: { 
      ...
      default: ['online', 'in_clinic'],
      validate: {
        validator: types => types && types.length > 0,
        message: 'At least one consultation type is required'
      }
    },
    availableDays: [{ 
    ...
    isActive: { type: Boolean, default: true, select: false }, // soft delete
...
doctorProfileSchema.index({ rating: -1, reviewCount: -1 });
doctorProfileSchema.index({ location: '2dsphere' });
// ❌ Removed compound index with parallel arrays
...
doctorProfileSchema.pre('save', function(next) {
  if (this.isModified()) {
    ...
    if (this.specialization) completedFields++;
    if (this.qualifications?.length > 0) completedFields++;
    if (this.consultationFee !== undefined) completedFields++;
    if (this.clinicName) completedFields++;
    if (this.clinicAddress?.street) completedFields++;
    if (this.contactInfo?.phone) completedFields++;
    if (this.availableDays?.length > 0) completedFields++;
    if (this.dailyTimeWindows?.length > 0) completedFields++;
    if (this.experienceYears !== undefined) completedFields++;
    if (this.verificationDetails?.documents?.length > 0) completedFields++;
